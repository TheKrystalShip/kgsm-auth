...
internal static class SigningKeyStore
{
    ...
    /// </para>
    /// <para>
    /// <b>Only the member holding the capability may write here.</b> A path carries no statement
    /// about who published it — unlike the gossiped fact, which a reader resolves through the holder
    /// — so an anchor standing by that wrote to it would hand every member on the machine a key that
    /// verifies nothing anybody signed with.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Remove a published key this anchor wrote, once it is no longer the one members should verify
    /// against. Returns whether a file was removed.
    /// </summary>
    /// <remarks>
    /// Removes only a file whose contents are this anchor's own key. A stale verification key is
    /// worse than none — members on the machine would go on accepting sessions signed by a member
    /// that has stood down — while a file holding a different key belongs to whoever holds the
    /// capability, and taking that away would break every member reading it.
    /// </remarks>
    internal static bool Withdraw(string path, string keysJson)
    {
        string full = Path.GetFullPath(path);
        if (!File.Exists(full) || File.ReadAllText(full) != keysJson)
            return false;
